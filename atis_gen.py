...
def atis_gen(atis_data):
    ...
    information=airport+AudioSegment.from_mp3("audio/airport_information.mp3")+AudioSegment.from_mp3("audio/"+atis_data["id"].lower()+".mp3")
    ...
    wind_speed=AudioSegment.empty()
    for i in wind_speed_input:
        wind_speed=wind_speed+AudioSegment.from_mp3("audio/"+i+".mp3")
    ...
    for i in atis_data["rwy"]:
        if i != " ":
            runway=runway+AudioSegment.from_mp3("audio/runway.mp3")+AudioSegment.from_mp3("audio/"+i[0]+".mp3")+AudioSegment.from_mp3("audio/"+i[1]+".mp3")
            if len(i)>2:
                s=i[2].lower()
                if s=='l':
...
#"sky": The sky conditions as they would be in a METAR formatted as an array, for example ('FEW100','OVC180'). Always append the string value written as " " to prevent errors.
#"rwy": The runways in use in an array, for example ('09R','09C','09L','13'). Always append the string value written as " " to prevent errors.
